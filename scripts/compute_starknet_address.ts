...
rl.question("Enter the class hash: ", (classHashInput) => {
    rl.question("Enter the salt (default: 0x65766d5f61646472657373): ", (saltInput) => {
        rl.question("Enter the deployer address (default: 1): ", (deployerInput) => {
            rl.close();

            const classHash = BigInt(classHashInput);
            const salt = saltInput.trim() ? BigInt(saltInput) : 0x65766d5f61646472657373n;
            const deployerAddress = deployerInput.trim() ? BigInt(deployerInput) : 1n;

...
impl LoggingOperations of LoggingOperationsTrait {
    ...
    fn exec_log0(ref self: Machine) -> Result<(), Errors> {
    ...
    fn exec_log1(ref self: Machine) -> Result<(), Errors> {
    ...
    fn exec_log2(ref self: Machine) -> Result<(), Errors> {
    ...
    fn exec_log3(ref self: Machine) -> Result<(), Errors> {
    ...
    fn exec_log4(ref self: Machine) -> Result<(), Errors> {
        internal::exec_log_i(ref self, 4)
    }
}

mod internal {
    use evm::errors::{Errors, EVMErrorEnum, InternalErrorEnum, WRITE_IN_STATIC_CONTEXT};
    ...
    fn exec_log_i(ref self: Machine, topics_len: u8) -> Result<(), Errors> {
        // Revert if the transaction is in a read only context
        if self.read_only() {
            return Result::Err(
                Errors::EVMError(EVMErrorEnum::WriteInStaticContext(WRITE_IN_STATIC_CONTEXT))
            );
        }

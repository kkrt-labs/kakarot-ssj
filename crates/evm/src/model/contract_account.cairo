...
impl ContractAccountImpl of ContractAccountTrait {
    ...
    /// Fetches the length of the bytecode of a contract account.
    fn fetch_bytecode_length(self: @Account) -> Result<usize, EVMError> {
        let contract_account = IContractAccountDispatcher {
            contract_address: self.address().starknet
        };
        Result::Ok(contract_account.bytecode_len())
    }

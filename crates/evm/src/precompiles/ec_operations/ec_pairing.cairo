...
pub impl EcPairing of Precompile {
    ...
    fn exec(mut input: Span<u8>) -> Result<(u64, Span<u8>), EVMError> {
        ...
        if n == 0 {
            return Result::Ok((BASE_COST, array![1].span()));
        }
        ...
        let gas = BASE_COST + n.into() * 34000;
...
mod tests {
    ...
    fn test_ec_pairing() {
        ...
        let (gas, return_data) = result.unwrap();
        assert_eq!(return_data.len(), 1);
        assert_eq!(*return_data.at(0), 1);
    }
}

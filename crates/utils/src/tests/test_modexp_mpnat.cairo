...
fn check_modpow_even(base: u128, exp: u128, modulus: u128, expected: u128) {
    ...
    let mut result = x.modpow(exp.to_be_bytes_padded().span(), ref m);
...
fn check_sub_to_same_size(a: u128, n: u128) {
    let mut x = MPNatTrait::from_big_endian(a.to_be_bytes().span());
    let mut y = MPNatTrait::from_big_endian(n.to_be_bytes().span());
...
fn check_is_odd(n: u128) {
    let mut mp = MPNatTrait::from_big_endian(n.to_be_bytes().span());

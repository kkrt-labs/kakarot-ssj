...
pub impl I256PartialOrd of PartialOrd<i256> {
    ...
    fn lt(lhs: i256, rhs: i256) -> bool {
        let lhs_positive = lhs.value.high < pow!(2, 127);
        let rhs_positive = rhs.value.high < pow!(2, 127);
...
fn i256_signed_div_rem(a: i256, div: NonZero<u256>) -> (i256, i256) {
    ...
    let a_positive = a.value.high < pow!(2, 127);
    let a = if a_positive {
        a
    } else {
        i256_neg(a).into()
    ...
    let div_positive = div.value.high < pow!(2, 127);
    div = if div_positive {
        div
    } else {
        i256_neg(div).into()
